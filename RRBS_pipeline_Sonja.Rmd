...
# grep all the chromosomes and make a list
...
### The full pipeline:
RRBS_CGpipeline_Parus.sh script
```{}
#!/bin/bash
...
for sample in $(cat /dss/dsslegfs01/pr53da/pr53da-dss-0024/projects/2021_EpigeneticsTits/Bisulfate_Conversion/Genomes/Parus.major.v1.1/Libraries.list); do sbatch -J ${sample} RRBS_CGpipeline_Parus.sh ${sample}; done 
```
...
# make a neat function to reorder columns (for later any analysis also)
...
# extract sample name and read group name
...
#convert bismark bam file to cgmap format 
cgmaptools convert bam2cgmap -b ${workdir}/${RUN}_trimmed_bismark_bt2_sort_RG.bam -g ${genomedir}/Parus.major_genome_Abel_Illumina.HiSeq_AllPathsv.April2013_v1.1.fa -o ${workdir}/${RUN}_trimmed_bismark_bt2_sort_RG.bam

#using bayesWC mode, call snps with an error rate of 0.01 (default 0.05) and a dynamic p-value based on coverage 
zcat ${workdir}/${RUN}_trimmed_bismark_bt2_sort_RG.bam.ATCGmap.gz | cgmaptools snv -m bayes -o ${workdir}/${RUN}.bayes.snv --bayes-e 0.01 --bayes-dynamicP -v ${outdir}/${RUN}_trimmed_bismark_bt2_sort_RG.bam.ATCGmap.vcf

# call SNPs with BS-SNPer, compress and index them
#BS-Snper.pl --fa ${genomedir}/Parus.major_genome_Abel_Illumina.HiSeq_AllPathsv.April2013_v1.1.fa ${workdir}/${RUN}_trimmed_bismark_bt2_sort_RG.bam --output ${workdir}/${RUN}_trimmed_bismark_bt2_sort_RG_snp.candidate.out --methcg ${workdir}/${RUN}_trimmed_bismark_bt2_sort_RG_meth.cg --methchg ${workdir}/${RUN}_trimmed_bismark_bt2_sort_RG_meth.chg --methchh ${workdir}/${RUN}_trimmed_bismark_bt2_sort_RG_meth.chh --minhetfreq 0.01 --minhomfreq 0.85 --minquali 10 --mincover 10 --maxcover 200 --minread2 2 --errorate 0.02 --mapvalue 20 >${outdir}/${RUN}_trimmed_bismark_bt2_sort_RG_SNP.vcf 2>${workdir}/${RUN}_SNP.log

# call SNPs with biscuit, compress and index them
...
bgzip ${outdir}/${RUN}_trimmed_bismark_bt2_sort_RG.bam.ATCGmap.vcf

tabix -p vcf ${outdir}/${RUN}_trimmed_bismark_bt2_sort_RG.bam.ATCGmap.vcf.gz
```
